...
class TestFetchFiiDii:
    def test_success_first_try_returns_parsed_data(self, mock_fii_dii_raw):
        with patch("services.orchestrator.premarket.premarket_report.requests.get",
    ...
    def test_retries_on_http_500(self, mock_fii_dii_raw):
        ...
        with patch("services.orchestrator.premarket.premarket_report.requests.get",
            ...
            with patch("services.orchestrator.premarket.premarket_report.time_module.sleep"):
                result = PreMarketReport()._fetch_fii_dii()
        assert result is not None

    def test_returns_none_after_all_retries_exhausted(self):
        fail = mock_response(None, 500)
        with patch("services.orchestrator.premarket.premarket_report.requests.get", return_value=fail):
            with patch("services.orchestrator.premarket.premarket_report.time_module.sleep"):
                result = PreMarketReport()._fetch_fii_dii()
    ...
    def test_sleep_called_between_retries(self):
        fail = mock_response(None, 500)
        with patch("services.orchestrator.premarket.premarket_report.requests.get", return_value=fail):
            with patch("services.orchestrator.premarket.premarket_report.time_module.sleep") as mock_sleep:
...
class TestFetchNsePreopen:
    ...
    def test_nse_urlfetch_called_once(self, sample_preopen_data):
        with patch("services.orchestrator.premarket.premarket_report.nse_urlfetch",
