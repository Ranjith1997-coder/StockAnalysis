...
- 15 new tests: `tests/pinn/test_run_training.py` (4, Redis-write behavior across the early-failure/accepted/rejected paths) + `tests/services/test_pinn_trainer_main.py` (11, day-idempotency checks, the training cycle, alert formatting — the scheduling loop itself is integration-level, same convention as `test_paper_trading_main.py`).
...
2. **Deploy `stockanalysis-pinn-trainer.service` to the server** — built and tested (0.4.4) but not yet installed/enabled there; training remains manual (CLI) until it is.
