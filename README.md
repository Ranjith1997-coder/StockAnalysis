...
| `stockanalysis-pinn-trainer.service` | 24/7 (self-scheduling) | **Not yet deployed to the server** (needs CPU torch — `make install-ml` / `requirements-ml.txt` — in the server venv first). PINN volatility model training — self-schedules once/day at 21:00 on trading days only (after positional analysis), retries every 15 min until 23:00 if Bhavcopy isn't published yet, writes `pinn:model:{symbol}` + Telegram alerts. See `docs/pinn-volatility-engine.md` section 0.4.4 |
