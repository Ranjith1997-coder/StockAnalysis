...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "<b>Paper Trading</b>\n"
        "/paper_positions — List open paper positions with leg details\n"
        "/paper_pnl — Account summary (P&amp;L, margin, daily stats, win rate)\n"
        "/paper_trades <code>[N]</code> — Recent N closed trades (default 10)\n"
        "/paper_close <code>[id|all]</code> — Close position(s)\n"
        "/paper_config <code>[key] [val]</code> — View/set paper config\n"
        "/paper_reset — Reset paper account (requires confirmation)\n"
        "/paper_export <code>[SYM]</code> — Export trades as CSV file\n\n"
